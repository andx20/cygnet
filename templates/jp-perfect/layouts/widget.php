...
else if (in_array($widget->position, array('toolbar-r' ,'toolbar-l','absolute', 'footer-l', 'footer-r', 'offcanvas'))) {
	$panel = 'uk-panel';
}
...
// Set title
if (in_array($widget->position, array('headerbar','toolbar-r' ,'toolbar-l','absolute', 'footer-l', 'footer-r'))) {
	$title = '';
...
if ($widget->menu) {
	...
	} else if (in_array($widget->position, array('toolbar-l','submenu', 'toolbar-r','toolbar-r','footer-l', 'footer-r', 'footer'))) {
		$renderer = 'subnav';
...
// Render widget
if (in_array($widget->position, array('breadcrumbs', 'logo-center', 'logo-small', 'logo-left', 'search', 'debug')) || (($widget->position == 'offcanvas') && $widget->menu)) {
	echo $content;
	...
	if (isset($params['animation'])) $animation = 'data-uk-scrollspy="{cls:\''.$params['animation'].'\''.((isset($params['animation_repeat'])) ?', repeat: true':'').'}"';
    else $animation = "";
